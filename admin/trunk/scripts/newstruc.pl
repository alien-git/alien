...
$db->do("CREATE TABLE GRPS (gId MEDIUMINT not null PRIMARY KEY AUTO_INCREMENT, user varchar(20) not null UNIQUE ) CHARACTER SET latin1 COLLATE latin1_general_cs");
...
foreach my $row (@$indexTable) {
  $table="L".$row->{tableName}."L";
  print "$table\n";
  $db->do("ALTER TABLE $table ADD (ownerId MEDIUMINT , gownerId MEDIUMINT), ADD FOREIGN KEY (ownerId) REFERENCES USERS(uId),ADD FOREIGN KEY (gownerId) REFERENCES GRPS(gId)" , {timeout=>[60000]} );
  ...
  $db->do("UPDATE $table JOIN USERS ON $table.owner=USERS.user JOIN GRPS ON $table.gowner=GRPS.user SET $table.ownerId=USERS.uId, $table.gownerId=GRPS.gId",{timeout=>[60000]} );
  #  or $db->do("UPDATE $table JOIN USERS ON BINARY $table.owner=USERS.user SET $table.ownerId=USERS.uId") ;
  # $db->do("UPDATE $table JOIN GRPS ON $table.gowner=GRPS.user SET $table.gownerId=GRPS.gId") 
  #  or $db->do("UPDATE $table JOIN GRPS ON BINARY $table.gowner=GRPS.user SET $table.gownerId=GRPS.gId");
