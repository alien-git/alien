...
system("$ENV{ALIEN_ROOT}/scripts/mysql_install_db", "--datadir=$mysqlDir/mysql", "--skip-name-resolve ", "--basedir=$ENV{ALIEN_ROOT}") and print "Error creating the empty database\n" and exit(-2);
