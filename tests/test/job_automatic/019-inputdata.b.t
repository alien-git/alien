...

{
  ...
  ($files->{"file.out"} eq "$content\n")
	or print "ERROR the content of the file '"
	. $files->{"file.out"}
	. "' does not match with the original '$content\n'\n"
	and exit(-2);
  ...
  grep (m{Getting alien-job-$id/.*/job-log/execution.out}, @log)
	...
	and exit(-2);
  grep (m{Getting .*/bin/}, @log)
	or print "We didn't downloaded any executable!!!\n" and exit(-2);
  ...
  $jdl =~ /$name/i
	or print "The requirement '${name}' is not there!!\n" and exit(-2);
