...
";
  ...
  addFile($cat, "${name}TestDisk1", $content, 'r', {options => "disk=1", check => "1"})
	or exit(-2);

  addFile($cat, "${name}TestDisk2", $content, 'r', {options => "disk=2", check => "2"})
	or exit(-2);

  addFile($cat, "${name}TestTape1", $content, 'r', {options => "tape=1", check => "1"})
	or exit(-2);
