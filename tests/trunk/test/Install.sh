...
PACKAGE_TESTS_LIST="$PACKAGES_TESTS_LIST job_automatic/090-executeAllJobs"
JOB_QUOTA_TESTS_LIST="$JOB_QUOTA_TESTS_LIST job_automatic/090-executeAllJobs"

USER_TESTS_LIST="$USER_BASIC_TESTS_LIST $CATALOGUE_TESTS_LIST $TRANSFERS_TESTS_LIST $JOB_TESTS_LIST $FILE_QUOTA_TESTS_LIST $PACKAGE_TESTS_LIST $JOB_QUOTA_TESTS_LIST 68-dbthreads"
...
GET_ARGUMENTS()
{
	...
	case $1 in
	    ...
	    -JOB_TESTS|-job_tests)
	        JOB_TESTS=1
		;;
	    -JOB_QUOTA_TESTS|-job_quota_tests)
	        JOB_QUOTA_TESTS=1
		;;
