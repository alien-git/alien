...

{
  ...
  my ($fileoutput) = $filecat->execute("get", "-silent", "$file")
	or print "ERROR: We could not get the file.\n" and exit(-2);
  ...
  ($fileoutput) =
	    $filecat->execute("get", "-x", "$file")
	and print "ERROR: We were still able to get the file.\n"
	and exit(-2);
...
#  $filecat->execute("add", "$file", $name , "${vo}::cern::exclusiveSE") and print "ERROR: We are still able to write on the SE.\n" and exit(-2);
  $filecat->execute("add", "$file", $name, "${vo}::cern::otherSE")
	and print "ERROR: We are still able to write on the SE.\n"
	and exit(-2);
