...
addFile(
  $cat, "editfile.txt", "This file is going to be edited with vi
Will it work??
", "r"
) or exit(-2);

my ($file) = $cat->execute("get", "editfile.txt") or exit(-2);
open(my $FILE, "<", $file) or exit(-2);

grep (/FILE_MODIFIED/, <$FILE>)
  and print "This is not the file that I want :(\n"
  and exit(-2);
close $FILE;
...
grep (/FILE_MODIFIED/, <$FILE>)
  or print "The file hasn't been modified :(\n" and exit(-2);
close $FILE;
