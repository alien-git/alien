...

{
  ...
  open(my $FILE, "<", $out) or print "Error opening $out" and exit(-2);
  my @data = <$FILE>;
  close $FILE;
  print "Got @data\n";

  grep (/Setting the environment to execute MyPS/, @data)
	or print "Error the package MyPS is not  initialized!!\n" and exit(-2);
  grep (/Setting the environment to execute MyLS/, @data)
	or print "Error the package MyLS is not  initialized!!\n" and exit(-2);
