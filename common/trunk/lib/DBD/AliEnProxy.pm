...
package DBD::AliEnProxy::dr; # ====== DRIVER ======

$DBD::AliEnProxy::dr::imp_data_size = 0;
...
#    }
    ...
    return DBD::AliEnProxy::proxy_set_err($drh, $err) if $err; # Returns undef

    my $proto   = getprotobyname('tcp');

    my $localhost = $ENV{'ALIEN_HOSTNAME'}.".".$ENV{'ALIEN_DOMAIN'};
    chomp $localhost;

    if ( $localhost eq $attr{'hostname'} ) {
    ...
    my $vf= new AliEn::Authen::ClientVerifier($socket,$user,$localuser,$attr{'forced_method'});

    $vf or return 0;
    
    my ($mes, $passwd, $cipherkey)=$vf->verify($auth,$attr{'PASSWD'});

    my $userCipherRef = undef;

    # If $cipherkey is set, use encryption (DES)
    if ($cipherkey) {
	my $cipher = "Crypt::DES";#$attr{'usercipher'} || $attr{'cipher'};
	$cipherRef = eval { $cipher->new(pack("H16", $cipherkey)) };
	if ($@) { $err .= " Cannot create usercipher object: $@."; print STDERR "Error: $err\n";}
    }

    if (!$mes)
    {
      #Passwd will contain an error message if ann error occured.
      print STDERR "An error occured during AliEn authentication:\t$passwd\n";
      my $error=($passwd || "Not able to authenticate");
...
$DBD::AliEnProxy::st::imp_data_size = 0;
...
# Note: Attribute names starting with 'proxy_' always treated as 'inherited'.
#
%ATTR = (	# see also %ATTR in DBD::AliEnProxy::db
    %DBD::AliEnProxy::ATTR,
    'Database' => 'local',
    'RowsInCache' => 'local',
    'RowCacheSize' => 'inherited',
...
DBD::AliEnProxy - A proxy driver for the DBI
...
DBD::AliEnProxy is a Perl module for connecting to a database via a remote
DBI driver.
...
database connect/disconnect overhead for the DBD::AliEnProxy
...
(note the semicolon) then DBD::AliEnProxy will create a new cipher object
by executing
...
The DBD::AliEnProxy driver supports this attribute (which is DBI standard,
