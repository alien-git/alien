...
package Authen::AliEnSASL::Perl::Client::JOBTOKEN;
...
sub new {
    ...
    $self->{host}     = shift;
    bless $self, $class;
    $self->log("WE HAVE STARTED A JOBTOKEN\n");
    use Data::Dumper;
...
sub step {
  ...
  my $in    = shift;
  my $inLen = shift;
  if ( $in =~ /^TOKENSASL OK( (\S+))?$/ ) {
    my $value="";
    $2 and $value=$2;

     $ENV{ALIEN_JOBTOKEN_USER}=$value;
    return ( $self->SASL_OK, $value, 0 );
  }
  else {
