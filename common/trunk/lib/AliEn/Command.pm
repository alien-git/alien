...
sub Save {
    ...
    my $localdir = ""
      ; #sprintf("%s/V%s/%05.5d/%05.5d",$self->{round},$ENV{ALIROOT_VERSION},$self->{run},$self->{event});
    my $lsfile;
    my $allregistered = 1;
	...
	foreach (@lsfiles) {
	    ...
	    my $localdone = $self->Register( $file, "$_->{'name'}", "$_->{'pfnoptions'}",$localdir,$selist ) or print STDERR "Cannot register |$_->{'extname'}|->|$file|\n";
	    if (! $localdone) {
...
sub Validate {
    ...
    my $host=shift;
    my $port=shift;
    ($port) or print STDERR "Error: not enough arguments in AliRoot Validate!!\nUsage: Validate <queueId> <HostMonitor> <port>\n" and return;
...
sub ParseOutput{
    ...
    foreach $file (@files)
    {
      if (!($self->{CATALOGUE}->execute("get", "/proc/$self->{QUEUEID}/$file",
					"$dir/$file")) ){
	...
	if (! (grep  (/$se/, @where)))
	{
	    ...
	    $self->{CATALOGUE}
	    ->execute("update", "/proc/$self->{QUEUEID}/stdout", 
		      "-pfn","soap://$se_address$dir/stdout?URI=SE", 
		      "-se", "$se");
	    $self->{CATALOGUE}
	    ->execute("update", "/proc/$self->{QUEUEID}/stderr", 
    ...
    my @possibleErrors=();
    $self->{STDOUT_ERRORS} and  
	@possibleErrors=@{$self->{STDOUT_ERRORS}};
    ...
    @possibleErrors=();
    $self->{STDERR_ERRORS} and  
	@possibleErrors=@{$self->{STDERR_ERRORS}};
    foreach my $error (@possibleErrors){
	@errors=(@errors,  grep( /$error/ , @file));
    }
...
sub ValidateUpdateDatabase{
    ...
    my $database=new AliEn::Database({"DB" =>"$config->{QUEUE_DATABASE}",
...
# Create a request
    ...
    $req->header(Subject => "Job $self->{QUEUEID} at $self->{CM_ADDRESS} finished with $status (validation $ENV{ALIEN_PROC_ID})");
