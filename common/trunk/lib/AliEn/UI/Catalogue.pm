...
my (@normalUser) = (
  'add',         'cd',           'chown',  'exit',   'find',     'get',
  'help',        'ls',           'mkdir',  'pwd',    'quit',     'rm',
  'rmdir',       'user',         'whoami', 'groups', 'chmod',    'umask',
  'cat',         'cp',           'tree',   'zoom',   'addTag',   'showTags',
  'addTagValue', 'showTagValue', 'echo',   'host',   'register', 'whereis',
  'history'
);
my $attribs;
...
sub help {
  ...
  if ($command) {
    $commands{$command}
      or $self->info("Command '$command doesn't exist. Type 'help' to get  a list of all the available commands")
      and return;
...
sub checkEnvelopeCreation {
  ...
  foreach my $var (
    'SEALED_ENVELOPE_LOCAL_PRIVATE_KEY',  'SEALED_ENVELOPE_LOCAL_PUBLIC_KEY',
    'SEALED_ENVELOPE_REMOTE_PRIVATE_KEY', 'SEALED_ENVELOPE_REMOTE_PUBLIC_KEY'
    ) {
    defined $ENV{$var} or return;
    (-f $ENV{$var})
      or $self->info("Warning: $var points to $ENV{$var}, but that file does not exist")
      and return;
  }
...
sub close {
  my $self = shift;
  if ($self->{CATALOG}) {
    eval { $self->{CATALOG}->f_disconnect; };
    if ($@) {
...
sub f_registerFile {
  ...
  (not $destSE and $pfn)
    and $self->info("Warning! The SE is not defined, but we are trying to store a $pfn... it won't work");
...
\t\tfile://<host>/<path>\nIf the method and host are not specified, the system will try with 'file://<localhost>'"
  ...
  return $self->{CATALOG}->authorize(
    "register",
    { lfn      => $self->{CATALOG}->GetAbsolutePath($file),
      pfn      => $pfn,
      size     => $size,
      md5      => $options->{md5},
      guid     => $guid,
...
sub access {
  ...
  if (!$newhash[0]->{envelope}) {
    ...
    ($newhash[0]->{exception})
      and return ({error => $error, exception => $newhash[0]->{exception}});
    return (0, $error);
  }
