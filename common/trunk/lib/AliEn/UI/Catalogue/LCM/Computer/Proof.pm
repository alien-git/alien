...
sub proofSetMasterPwd {
    ...
    print "\n";
    system "stty echo";
    my $pw = crypt("$word","salt"); print "Crypted Password is: $pw\n";
	...
	$self->{SOAP}->checkSOAPreturn($response, "IS") or $self->{LOGGER}->error("Proof","IS could not find the Service User of the TcpRouter for $site\n") and next;
	$response = $response->result;
	$response or $self->{LOGGER}->error("Proof","IS could not find the Service User of the TcpRouter for $site\n") and next;
