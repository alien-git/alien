...
sub registerOutput {
  ...
  my $options = {};
  @ARGV = @_;
  Getopt::Long::GetOptions($options, "cluster")
  ...
  (my $jobinfo) = $self->execute("ps", "jdl", $jobid, "-dir", "-status", "-silent")
    or $self->info("Error getting the jdl of the job", 2)
    and return;
  ...
  if ($user ne $currentuser) {
    $self->execute("user", "-", $user)
      or $self->info(
"Error, you are not the user the job belongs to. registerOutput can be only called by the job owner '$user' and you are not allowed to become '$user'.",
      2
      ) and return;
  }
  if (!$onlycmlog) {
    ...
    $outputdir and $self->info("The output files were registered in $outputdir")
      or $self->info("Error during output file registration.")
      and return;
  }
  ...
  if (!$onlycmlog) {
    ...
    $self->{TASK_DB}
      or $self->info("Error CE: In initialize creating TaskQueue instance failed", 2)
      and return;
