...
sub checkConfigFile {
  ...
  foreach ( $config->get() ) {
    ...
    if ( grep ( /^$key$/, @SERVICES ) ) {
      $DEBUG
        and $self->debug( 2,
                      "Ignoring the block '$key' from the configuration file" );
      next;
    }
    my $block;
    eval {
...
"We are supposed to get the configuration from ALIEN_LDAP_DN=$ENV{ALIEN_LDAP_DN}, but this doesn't look like our organisation $self->{ORG_NAME})"
  ...
  eval {
    ...
    $DEBUG
      and
      $self->debug( 1, "Getting the Config of $self->{ORG_NAME} from $base" );
...
"Error getting the configuration for the organisation from host=$self->{LDAPHOST} and dn=$self->{LDAPDN}\n"
  ...
  foreach $attr ( $entry->attributes ) {
    ...
    $DEBUG
      and $self->debug( 7,
        "Setting $value as ($attr)  " . $entry->get_value($attr) . " (@list)" );
  }
...
sub GetHostConfig {
  ...
  my @variablesName =
    ( "LOG_DIR", "TMP_DIR", "CACHE_DIR", "WORKDIR", "LOCAL_CONFIG" );
  foreach (@variables) {
  ...
  if (@se) {
    ...
    foreach my $serviceName (@se) {
      ...
      $se
        ...
        "Config",
"Error host  '$host' is supposed to be close to $serviceName, but that SE does not exist"
        ) and return;
...
sub CheckUser {
  ...
  foreach my $attr ( $entry->attributes ) {
    ...
    $DEBUG
      and
      $self->debug( 7, "Putting $attr as $value\n\tAnd ${attr}_LIST=@list" );
    $hash->{"$attr"}        = $value;
...
sub GetgContainer {
  ...
  for ( my $i = 0 ; $i < $total ; ++$i ) {
    ...
    my $judgeBase =
"ou=Judges,name=$serviceName,ou=Services,ou=gContainer,ou=Services,$self->{LDAPDN}";
    ...
    if (
       grep( /AliEngContainerServiceGAS/, @{ $service->{OBJECT_CLASS_LIST} } ) )
    {
      $self->debug( 1, "Getting the GAS Modules" );
      my $modulesFilter = "(&(objectClass=AliEnGASMODULE))";
      my $modulesBase =
"name=$serviceName,ou=Services,ou=gContainer,ou=Services,$self->{LDAPDN}";
      ...
      for ( my $i = 0 ; $i < $modulesCount ; ++$i ) {
        ...
        $service->{GAS_MODULES_HASH}->{ $entry->get_value("alias") } =
          $self->getAttributes( $entry, 1 );
      }
    }
