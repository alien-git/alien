...
sub f_queue_list {
  ...
  if (@$array) {
    $self->info("----------------------------------------------------------------------------------------------------",undef,0);
    $self->info(sprintf("%-32s %-12s %-20s %5s %5s %4s/%-4s %4s/%-4s","site","open", "status", "load", "runload", "queued","max", "run", "max"),undef,0);
    foreach (@$array) {
      ...
      $self->info(sprintf("%-32s %-12s %-20s %5s %5s %4s/%-4s %4s/%-4s",$_->{'site'},$blocked,$_->{'status'},$queueload,$runload,$allqueued,$maxqueued,$allrunning, $maxrunning),undef,0);
      $s1+=$allqueued;
      $s2+=$maxqueued;
      $s3+=$allrunning;
      $s4+=$maxrunning;
    ...
    my $sumsite="All";
    $self->info("----------------------------------------------------------------------------------------------------",undef,0);
    $self->info(sprintf("%-32s %-12s %-20s %5s %5s %4s/%-4s %4s/%-4s",$sumsite,$empty,$empty,$s5,$s6, $s1,$s2,$s3,$s4),undef,0);
    $self->info("----------------------------------------------------------------------------------------------------",undef,0);
  }
...
#   check for the -f 'fix' flag, which resubmits all faulty jobs ....
  ...
  if (($_[0] eq '-k') or  ($_[0] eq '-q') ) {
    ...
    foreach (@allps) {
      ...
      ($_[0] eq '-q') and ($status != /^Z|W|(EE)|(EA)|(ES)|(ER)|Q|(ESV)|(EV)|(EVT)|(EVN)|(EIB)$/ ) 
	and next;
      my $id2kill = $id;
      $id2kill =~ s/\-//g;
...
sub calculateJobQuota {
 ...
 ( $user =~ /^admin(ssl)?$/ ) or
  ...
  $self->{TASK_DB}->do("update PRIORITY pr left join (select SUBSTRING( submitHost, 1, POSITION('\@' in submitHost)-1 ) as user, sum(p.cost) as totalCpuCostLast24h from QUEUE q join QUEUEPROC p using(queueId) where (unix_timestamp()>=q.received and unix_timestamp()-q.received<60*60*24) group by submithost) as C on pr.user=C.user set pr.totalCpuCostLast24h=IFNULL(C.totalCpuCostLast24h, 0)") or $self->$method(@data, "Failed");
