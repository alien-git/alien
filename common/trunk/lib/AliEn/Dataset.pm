...
sub readxml {
  ...
  # parse by hand
  open XMLIN, "$xml"
    or $self->info("Error opening the file $xml")
    and return;
  my $ptr;
  my $cnt = 0;
  ...
  while (<XMLIN>) {
    ...
    if ($line =~ /event[\s]*name=\"([^\"]*)\"/) {
      ...
      $ptr = "$1";
      my $filehash;
      $self->{XMLhash}->{collection}->{event}->{"$ptr"}->{file} = $filehash;
      next;
    }
...
sub split {
  ...
  for (@eventsToProcess) {
    ...
    for (@$_) {
      ...
      if (
        $#{$eventGroup} > -1
        and (($paramMaxEvents != 0 and $numberOfEvents + 1 > $paramMaxEvents)
          or ($paramMaxSize != 0 and $totalFileSize + $_->{totalFileSize} > $paramMaxSize))
        ) {
...
sub getAllLFN {
  my $self = shift;
  my @list = ();
  $self->{XMLhash} and $self->{XMLhash}->{collection} and $self->{XMLhash}->{collection}->{event}
    or return;
