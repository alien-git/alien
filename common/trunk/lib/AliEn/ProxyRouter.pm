...
sub Multiplexer {
    ...
    $self->{MASTERSOCKET} = new IO::Socket::INET (LocalHost => $lhostname,
				LocalPort => $self->{PORT},
				Proto     => 'tcp',
				Listen    => $self->{LISTEN},
				Reuse     => $self->{REUSE},
	...
	foreach $socket (@new_handles) {
		...
		do {
		    ...
		    for $checkport ( $portstart .. $portstop ) {
		      $newforwardsocket = IO::Socket::INET->new(Proto     => "tcp",
								PeerAddr  => $fwhost,
								PeerPort  => $checkport);
		...
		push @{$self->{MUX}} , {rsocket         => $newsocket,
