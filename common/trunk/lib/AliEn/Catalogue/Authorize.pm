...
sub getSEforPFN {
  ...
  my $query = $self->{DATABASE}->{LFN_DB}->{FIRST_DB}->paginate(
"SELECT seName FROM SE WHERE seioDaemons LIKE concat ( ? , '%') AND ? LIKE CONCAT(seStoragePath, '%')  ORDER BY length(seStoragePath) DESC ",
    1, 0
  );
  my $sestring = $self->{DATABASE}->{LFN_DB}->{FIRST_DB}->queryValue($query, undef, {bind_values => \@queryValues});
...
sub getBaseEnvelopeForWriteAccess {
  ...
  if (!$envelope->{guid} or !AliEn::Util::isValidGUID($envelope->{guid})) {
    ...
    if ($user ne "admin") {
      my $collision =
        $self->{DATABASE}->{LFN_DB}->{FIRST_DB}->queryRow(
