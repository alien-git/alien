...
sub checkConfigFile {
  ...
  foreach ($config->get()) {
	...
	$self->{$name} or
	  $self->info("The local configuration defines '<$key> $subkey=".$subconfig->get($subkey)."</$key>' (a variable that is not in the standard configuration)");
...
sub GetPackages {
	...
	grep(/^${pack}::$version$/, @newlist) or
	    @newlist=(@newlist, "${pack}::$version");
      }
...
#  print "looking for the version of $package";
  ...
  if ( !$total ) {
    ( !$self->{SILENT} )
      and print STDERR
	"Warning: LDAP package '$package' does not exist in $self->{ORG_NAME}\n";
    return;
  }
...
sub addPackage {
    ...
    if ( !$total ) {
        ( !$self->{SILENT} )
          and print STDERR
"Warning: LDAP package '$name' (version '$version') does not exist in $self->{ORG_NAME}\n";
        return;
    }
...
sub GetConfigFromService {
  ...
  if ($service eq "ClusterMonitor") {
    ...
    ($host and $port) or print STDERR "ERROR: The environment variable ALIEN_CM_AS_LDAP_PROXY was set ($ENV{ALIEN_CM_AS_LDAP_PROXY}), but not with a host:port syntax!!\n" and return;
...
sub CheckService{
  ...
  $service =~ /^((SE)|(CE)|(FTD)|(CLC)|(TCPROUTER)|(PACKMAN))$/ or
