...
sub initialize{
  ...
  $self->checkTable("TODELETE",  "guid", {guid=>"char(36) primary key",
					 pfn=>"varchar(255)"}
		   ) or return;

  $self->checkTable("BROKENLINKS",  "guid", {guid=>"char(36) primary key"})
    or return;

  $self->checkTable("FILES2",  "guid", {guid=>"char(36) primary key"})
    or return;
...
sub chooseVolume {
  ...
  if ($guid){
    my $volumes=$self->queryColumn("SELECT concat('volumeId !=', volumeId) from FILES where guid='guid'");
...
sub getPFNFromGUID{
  my $self=shift;
  my $guid=shift;
  return $self->queryColumn("SELECT pfn from FILES where guid='$guid'")
