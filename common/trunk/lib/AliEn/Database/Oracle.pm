...
sub getConnectionChain {
	...
	defined $ENV{'ORACLE_SID'}
	  or $self->info(
"The Oracle SID is not defined in your system. Normally it should be. We take 'alien' as ORACLE_SID by convention"
	  );
	my $db = $ENV{ORACLE_SID} || "alien";

	( $db, $self->{SCHEMA} ) = split( ":", uc $self->{DB} );
	if ( uc( $self->{SCHEMA} ) ne uc( $self->{ROLE} ) ) {
		if (    $self->{ROLE} !~ /^admin(ssl)?/i
			and $self->{SCHEMA} =~ /^admin?/i )
		{
			print STDERR "Only the administrator can access the admin schema\n";
			return;
		}
		elsif ( $self->{ROLE} !~ /^admin(ssl)?/i
			and $self->{SCHEMA} !~ /^admin?/i )
		{
			$self->{SCHEMA} = 'alienstandard';
		}
	}
...
sub _connectSchema {
	...
	$self->{SCHEMA} =~ s/(.+):(.+)/$2/i;
	print "\n\nWe are connecting to this schema $self->{SCHEMA}";
...
sub grantPrivilegesToObject {
	...
	if ( $object =~ m/(.*)\*(.*)/i ) {
		...
		my $s =
		  $self->{DBH}->prepare(
"begin grant_whole_schema(\'$privs\',\'$schema_from\',\'$user_to\');end;"
		  );
		$s->execute;
		return 1;
...
sub multiinsert {
	...
	my $new_fields =
	  $self->preprocess_fields( \@fields );    #for the reserved words
	my @new_f = @$new_fields;
	...
	foreach $rloop (@$rarray) {
		...
		if (   $options->{ignore}
			&& $DBI::errstr =~ /ORA-00001: unique constraint/ )
		{
