...
my $my_faultSub=sub {my $soap=shift; 
		     my $message=shift;
...
sub new {
  ...
  $self->{NOPROXY}
    and $self->debug(1, "Not using the SOAPProxy") and
      return $self;
  my $host=$self->{CONFIG}->{SOAPPROXY_ADDRESS};
  $host or $self->debug(1, "No proxy defined in LDAP")
    and return $self;
  
  $self->debug(1, "Using the SOAPProxy");
  
  $self->{SOAPPROXY}= SOAP::Lite->uri("AliEn/Service/SOAPProxy")
    ->proxy("http://$host");
...
sub getPassphrase {
  ...
  my @data=<FILE>;
  close FILE and  $self->info("@data", undef, 0)
    and return "";
  ...
  ($ENV{X509_USER_CERT}, $ENV{X509_USER_KEY})=($ENV{HTTPS_CERT_FILE}, $ENV{HTTPS_KEY_FILE});
  $d=open (FILE,  "$ENV{GLOBUS_LOCATION}/bin/grid-proxy-init -pwstdin <<EOF
$passphrase
EOF|");
  ($ENV{X509_USER_CERT}, $ENV{X509_USER_KEY})= ($oldCert, $oldKey);    
...
sub exportSecureEnvironment {
  ...
  if (! system("$ENV{GLOBUS_LOCATION}/bin/grid-proxy-info >/dev/null 2>&1")) {
    ...
    foreach my $type ("CERT", "KEY") {
      ...
      $ENV{$name} = ($ENV{$name} || $ENV{"X509_USER_$type"}
		     || "$dir/user\L$type\E.pem");
      (-f $ENV{$name}) or 
	$self->info( "Warning! $ENV{$name} doesn't exit");
    }
...
sub Connect {
  ...
  eval  {
    ...
    $self->{$name}= SOAP::Lite->uri($uri)->proxy(@list)
...
#  ($service eq "Manager/Job" )and $configName="JOB_MANAGER";
  ...
  if ($self->{SOAPPROXY}) {
    ...
    $self->checkSOAPreturn($result) or return (-1, $self->{LOGGER}->error_msg);
    return 1;
  }
  ...
  if ( $self->{$service} ) {
    eval {
      $done = $self->_CallSOAPInternal($service, "ping")->result;
    };
  }
  ($done) and  
    $self->debug(1, "Connection is up") and return 1;
...
sub resolveSEName {
  ...
  ($self->{SE_CACHE}->{$sename} and
   $self->{SE_CACHE}->{$sename}->{expired}<$now) and 
     delete $self->{SE_CACHE}->{$sename};
  my $response;
  if (! $self->{SE_CACHE}->{$sename}) {
    ...
    $self->Connect({address=>"http://$remoteSE:$portSE",
