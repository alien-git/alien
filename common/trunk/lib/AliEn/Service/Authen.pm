...
my $SubjectToUid = sub {
  ...
  my $uid = $self->{addbh}->queryValue("SELECT user from USERS_LDAP where dn=?", undef, {bind_values => [$subject]});
  if (!$uid) {
...
sub changePrivileges {
  ...
  my $date = localtime;
  $date =~ s/^\S+\s(.*):[^:]*$/$1/;
  $self->info("Changing owner of $table in $db $host from $oldUser to $newUser");
  ...
  $self->{cat}->{DATABASE}->grantAllPrivilegesToUser($newUser, $db, "T$table")
    ...
    or $self->{LOGGER}->warning("Authen", "Error changeing privilege on $db.T$table to $newUser")
    and return;
...
sub insertKey {
  ...
  eval {
    if ($LDAP->modify("uid=$user,ou=People,$self->{CONFIG}->{LDAPDN}", replace => {'sshkey' => $key})) {
      $self->info("$user has succesfully updated is SSHKEY");
    } else {
  ...
  $self->info("$user requests insertion of public keys in database");
...
sub CheckLocalPassword {
  ...
  $self->checkUserDB($user)
    or return (0, "User $user does not exist in LDAP");
...
sub CheckProductionUser {
  ...
  my $data = $self->{addbh}->getDBKey($username)
    or $self->{LOGGER}->error("Authen", "Error fetching private key for user $username")
    and return;
  ...
  eval {
    ...
    my $total = $mesg->count;
    print STDERR "FOUND $total in ou=People,$self->{CONFIG}->{LDAPDN} with(uid=$username)\n";
    if ($total) {
...
sub insertCert {
  ...
  eval {
    ...
    if ($total > 0) {
      ...
      return (-1, "A certificate with subject $subject is alredy in LDAP server");

    }

    if ($LDAP->modify("uid=$user,ou=People,$self->{CONFIG}->{LDAPDN}", replace => {'subject' => $subject})) {
      $self->info("$subject mapped to AliEn user $user");

    } else {
      $self->{LOGGER}->warning("Authen", "Error in updating subject for $user");
      return (-1, "An LDAP error occured on serverside. Contact AliEn administrators");
    }
  };
  if ($@) {
    $self->info("Error doing the ldap query: $@");
...
sub addUser {
  ...
  $self->info("User $user does not exists in database. Creating user.");
...
sub requestCert {
  ...
  open(FILE, ">$file")
    or print STDERR "Error opening the file $file"
  ...
  system("$ENV{ALIEN_ROOT}/bin/openssl",
    "x509", "-in", "$file", "-noout", "-req", "-signkey", "$ENV{ALIEN_CA}/private/key.pem");
  ...
  $subject =~ /subject=\/C=ch\/ST=Switzerland\/L=Geneva\/O=AliEn\/O=$organisation\/OU=People\/CN=$username/i
    or print STDERR "Error $subject does not match with $username\n"
  ...
  my $command =
"$ENV{ALIEN_ROOT}/bin/openssl ca -batch -passin file:$path -config $config -policy policy_anything -out $outfile -in $file";
  ...
  open(FILE, "$outfile")
    or print STDERR "Error opening the file $outfile\n"
