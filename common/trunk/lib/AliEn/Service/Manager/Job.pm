...
sub changeStatusCommand {
  ...
  if ( $status eq "WAITING" ) {
    ...
    $set->{error} = $error;
  } elsif (( $status eq "SAVED" && $error)|| ($status eq "ERROR_V")) {
...
sub getTop {
  ...
  $all_status or $data->{status} or $data->{id} or $where.=" and ( status='RUNNING' or status='WAITING' or status='ASSIGNED' or status='QUEUED' or status='INSERTING' or status='STARTED' or status='SAVING' or status='SAVED' )";
