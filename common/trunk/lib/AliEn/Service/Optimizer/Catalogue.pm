...
sub checkSEdatabase {
  ...
  $se->do("INSERT IGNORE INTO TODELETE select f.guid, f.pfn from FILES f left join  FILES2 f2 on f.guid=f2.guid where f2.guid is null");
  $se->do("INSERT IGNORE INTO BROKENLINKS select f2.guid from FILES f right join  FILES2 f2 on f.guid=f2.guid where f.guid is null");
...
sub checkGUID {
  ...
  foreach my $host (@$hosts){
    ...
    foreach my $table (@$tablesRef){
      ...
      my $entries=$self->{DB}->queryColumn("SELECT entryId FROM $tableName WHERE ( (guid is NULL) or (guid = '') ) and lfn not like '%/' limit 10000");
	...
	$self->{DB}->update($tableName, {guid=>$guid},"entryId=$entry");
	$i++;
