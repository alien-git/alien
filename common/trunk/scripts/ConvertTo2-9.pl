...
if ($caseSensitive){
  ...
  print "We should also define the new functions\n";
  $db->do("create function string2binary (my_uuid varchar(36)) returns binary(16) deterministic sql security invoker return unhex(replace(my_uuid, '-', ''))") or return;
