...
#  $self->{COUNTER}++;
  ...
  $self->{DB}->do("delete from OCDB where failed >= 5");
...
#	open FILE, ">>", $self->{DIRFILES}."ocdb_${ocdbType}_".$time;
	...
	foreach my $lfn (@$lfnIds){
      ...
      if(!$localfile) {
      	...
      	$self->{DB}->do("update OCDB set failed=failed+1 where entryId=?",{bind_values=>[$lfn->{entryId}]});
      	next;
      }
...
#    close FILE;
    ...
    $error or $self->{DB}->do("delete from OCDB where entryId in (".join(',', map { '?' } @okLfns).")", {bind_values => [@okLfns]});
