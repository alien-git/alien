...
########################################################################
package AliEn::Logger::LogAgentDriverAliEn;
...
sub make {
	...
	$self->_init($prefix, 0);					# 0 is the skip Carp penalty
        ...
        $self->{'channels'} = {
            'raw'  => select((select(main::STDOUT), $| = 1)[0]),
            'output' => select((select(main::STDOUT), $| = 1)[0]),
            'error'  => select((select(main::STDERR), $| = 1)[0]),
        };

	#select((select(main::STDOUT), $| = 1)[0]);	# Autoflush
	return $self;
...
sub prefix_msg {
	...
	return $str if !defined($prefix) || $prefix eq '';
	#return "$prefix: " . $str;
	return "$prefix: " . $str;
}
...
sub write {
	...
	my ($channel, $priority, $logstring) = @_;
        #print "channel: $channel, prio: $priority, string: $logstring ...\n";
	local $\ = undef;
        if($channel eq "error") { 
	  print main::STDERR "$logstring\n";
        } elsif($channel eq "raw") {
          print main::STDOUT "$logstring";
        } else {
	  print main::STDOUT "$logstring\n";
        }
}
