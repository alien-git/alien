...
sub new {
  ...
  if ($options->{VIRTUAL}) {
    ...
    $self->{CONFIG}->{"SE_$options->{VIRTUAL}"} or $self->info("Error in MSS: the virtual SE $options->{VIRTUAL} doesn't exist") and return;
    $savedir=$self->{CONFIG}->{"SE_$options->{VIRTUAL}"}->{SAVEDIR};
    @environment=();
    $self->info("Checking the environment for a virtual SE");
    $self->{CONFIG}->{"SE_$options->{VIRTUAL}"}->{ENVIRONMENT_LIST} and 
      push @environment, @{$self->{CONFIG}->{"SE_$options->{VIRTUAL}"}->{ENVIRONMENT_LIST}};
  }
  ...
  ( defined $savedir )or $self->{LOGGER}
      ->warning( "MSS", "Error setting the MSS: no SAVEDIR specified" )
	and return;
  ...
  if ( $savedir ) {
    ...
    $self->mkdir( $self->{SAVEDIR} )
      and $self->{LOGGER}->warning( "MSS",
				    "Error creating the save directory $self->{SAVEDIR}\n" )
	and return;
  }
...
sub save {
    ...
    ( $source and $target )
      or $self->{LOGGER}
      ->warning( "MSS", "Error: not enough arguments to save a file" )
      and return;
...
sub createdir {
    ...
    $self->{SAVEDIR}
      ...
      and $self->{LOGGER}
      ->warning( "MSS:file", "Error creating $self->{SAVEDIR}/$dir" )
      and return;
    return 1;
}
...
sub getFTPCopy{
  ...
  if ($self->{FTP_LOCALCOPIES}){
	...
	and $self->{LOGGER}->error("MSS", "Error getting the local copy of $path") and return;
    $path=$localpath;
