...
sub submit {
  ...
  my $date = time;
  my $out  = AliEn::TMPFile->new(
    { base_dir => $self->{CONFIG}->{LOG_DIR},
      filename => "$ENV{ALIEN_LOG}.out"
    }
    )
    or $self->info("Error gettting the file name")
    and return -1;
...
sub getQueueStatus {
  ...
  $self->{GET_QUEUE_STATUS}
    or return "Sorry, not implemented for queue " . ref($self) . "\n";

  open(OUT, "$self->{GET_QUEUE_STATUS} |")
    or print "Error doing $self->{GET_QUEUE_STATUS}\n" and return "Error doing $self->{GET_QUEUE_STATUS}\n";
