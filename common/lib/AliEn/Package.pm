...
sub new {
  ...
  ($self->{PACKMAN})
    or print STDERR "Error creating the package: Package manager not received\n" and return;
  ...
  if ($self->{path} eq "auto") {
    ...
    $self->{path} = "$ENV{ALIEN_HOME}/packages/$self->{provide}->{package}/$self->{provide}->{version}";
...
sub CheckRequirements {
  ...
  $self->{PACKMAN}->{CONFIG}->{DEBUG} > 0
    and print "Requirements for $self->{provide}->{package}::$self->{provide}->{version} '@requirements'...";
...
#   print "ESTAMOS MIRANDO SI ESTA INSTALADO\nTENGO $self->{PACKMAN}->{CONFIG}->{CLUSTER_MONITOR_USER} y $self->{PACKMAN}->{CONFIG}->{LOCAL_USER}\n";
  ...
  my ($file) = $catalog->execute("aioget", "-n", "$fileName", "$self->{path}/Pac$$.tar.gz");
  $catalog->close;
