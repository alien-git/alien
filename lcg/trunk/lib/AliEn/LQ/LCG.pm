...
sub initialize {
   ...
   $ENV{X509_CERT_DIR} and $self->{LOGGER}->debug("LCG","X509: 
$ENV{X509_CERT_DIR}");
   $self->info("LQ::LCG checking for valid LCG proxy.");
   my $error = !defined $ENV{GLOBUS_LOCATION};
   $error or $error = system("$ENV{GLOBUS_LOCATION}/bin/grid-proxy-info >& 
/dev/null");
   $error or $error = !system("$ENV{GLOBUS_LOCATION}/bin/grid-proxy-info |grep 
\'0:00:00\'>& /dev/null");
   $error and return;
...
#
     ...
     if ($error) {
       $contact or $contact="";
       $self->{LOGGER}->warning("LQ::LCG","Error submitting the job. Log file 
'$contact'\n");
...
sub getStatus {
     ...
     if ( $LCGStatus eq "Done(Success)" )  {
        ...
        my ($contact )= $self->getContactByQueueID($queueid);
        $contact or return;
        $self->info("Will retrieve OutputSandbox for job $queueid, JobID is 
$contact");
...
sub getJobStatus {
   ...
   open( OUT, "/opt/edg/bin/edg-job-status -noint @args \"$contact\"| grep 
\"$pattern\"|" );
   my @output = <OUT>;
   close(OUT);
   my $status = $output[0];
...
other.GlueCEStateWaitingJobs)/other.GlueCEInfoTotalCPUs;
FuzzyRank = True;
...
test -f \$VO_ALICE_SW_DIR/aliroot/alice-env-ALICE-$self->{ALICETAG}.sh || (echo 
\"Error: environment file not found\"; exit 1;)
source \$VO_ALICE_SW_DIR/aliroot/alice-env-ALICE-$self->{ALICETAG}.sh
